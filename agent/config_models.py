...
class GCPStorageConfig(BaseModel):
    ...
    def validate_bucket_name(cls, v: str) -> str:
        ...
        if not v.replace("_", "").replace("-", "").replace(".", "").isalnum():
            raise ValueError(
                "bucket_name can only contain letters, numbers, hyphens, underscores, and dots"
            )
        return v
...
class AlphaVantageConfig(BaseModel):
    ...
    rate_limit_delay: int = Field(
        default=12, ge=1, description="Seconds between API calls"
    )
...
class RiskAnalysisConfig(BaseModel):
    ...
    analysis_period_days: int = Field(
        default=252, ge=1, description="Trading days for analysis"
    )
...
class InvestmentConfig(BaseModel):
    ...
    ticker_mappings: Dict[str, str] = Field(
        ..., description="Stock name to ticker symbol mappings"
    )
